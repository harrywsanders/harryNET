...
public:
    ...
    : weights(other.weights), m_weights(other.m_weights), v_weights(other.v_weights),
      ...
      type(other.type) {
    if (other.activationFunc) {
        activationFunc = other.activationFunc->clone();
    }
}
    ...
    Layer(size_t nNeurons, size_t nInputsPerNeuron, LayerType layerType = LayerType::Dense)
        : type(layerType)
    {
        activationFunc = std::make_unique<ReLU>();
    ...
    Layer(size_t nNeurons, size_t nInputsPerNeuron, std::unique_ptr<ActivationFunction> actFunc, LayerType layerType = LayerType::Dense)
        : type(layerType), activationFunc(std::move(actFunc)) {
        if (nNeurons == 0 || nInputsPerNeuron == 0) {
            throw std::invalid_argument("Number of neurons and inputs per neuron must be greater than 0.");
        }
        if (layerType == LayerType::Dense) {
